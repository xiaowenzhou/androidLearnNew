...
import cn.ktc.learnandroid.utils.DensityUtils;
...
public class MyClockView extends View {
    ...
    /*加一个默认的padding，为了防止用camera旋转时钟造成四周超出view大小*/
    ...
    public MyClockView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MyClock, 0, 0);
        mBackgroundColor = ta.getColor(R.styleable.MyClock_ClockView_clock_backgroundColor, Color.parseColor("#237EAD"));
        mLightColor = ta.getColor(R.styleable.MyClock_ClockView_clock_lightColo, Color.parseColor("#ffffff"));
        mDarkColor = ta.getColor(R.styleable.MyClock_ClockView_clock_darkColor, Color.parseColor("#80ffffff"));
        mTextSize = ta.getDimension(R.styleable.MyClock_ClockView_clock_textSize, DensityUtils.sp2px(context, 14));
        ta.recycle();
    ...
    private void getCurrentTime() {
        ...
        float millSecond = calendar.get(Calendar.MILLISECOND);
        float  second =calendar.get(Calendar.SECOND)+millSecond/1000;
    ...
    private void drawScaleLine() {
        ...
        //matrix默认会在三点钟方向开始颜色的渐变，为了吻合钟表十二点钟顺时针旋转的方向，把秒针的旋转角度减去90度
    ...
    private void  drawSecondNeedle(){
        ...
        mSecondPaint.setColor(mLightColor);
        mCanvas.drawPath(mSecondHandPath,mSecondPaint);
        mCanvas.restore();
    ...
    private void drawMinuteNeedle(){
        ...
        float offset = mPaddingTop+mTextRect.height()/2;
        mMinuteHandPath.moveTo(getWidth()/2-0.008f*mRadius,getHeight()/2-0.03f*mRadius);
        mMinuteHandPath.moveTo(getWidth()/2-0.008f*mRadius,offset+0.365f*mRadius);
        mMinuteHandPath.quadTo(getWidth()/2,offset+0.345f*mRadius,getWidth()/2+0.008f*mRadius,offset+0.365f*mRadius);
        mMinuteHandPath.lineTo(getWidth()/2+0.01f*mRadius,getHeight()/2-0.03f*mRadius);
        mMinutePaint.setStyle(Paint.Style.STROKE);
        mMinutePaint.setStrokeWidth(0.02f*mRadius);
        mCanvas.drawArc(mCircleRectF,0,360,false,mMinutePaint);
        mCanvas.restore();
